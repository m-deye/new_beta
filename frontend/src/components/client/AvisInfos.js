...
const JobOfferModal = ({ show, handleClose, offre }) => {
  ...
  //       `http://localhost:8000/avis_infos/api/offres/${formData.id}/`,
  //       formData,
  //       {
  ...
  const saveJobOffer = async () => {
    ...
    try {
      const response = await axiosInstance.put(
        `/avis_infos/api/offres/${formData.id}/`,
        formData,
        {
          headers: { "Content-Type": "application/json" },
        }
  ...
  const fetchDocuments = async () => {
    ...
    try {
      const response = await axiosInstance.get(
        `/avis_infos/api/documents/${offre.id}/`
      );
  ...
  const handleDocumentSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:8000/avis_infos/ajouter_document_client/${offre.id}/`,
        {
          method: "POST",
          headers: {
  ...
  const deleteDocument = async (documentId) => {
    ...
    try {
      const response = await axiosInstance.delete(
        `/avis_infos/api/documents/delete/${documentId}/`
      );
                  ...
                  <Col md={6} style={{ paddingRight: "20px" }}>
                    <Form.Group className="mb-4">
                      <Form.Label
                        ...
                        style={{ fontSize: "16px", color: "#858796" }}
                      >
                        Avis fixe <span style={{ color: "black" }}>*</span>
                      </Form.Label>
                      <Form.Select
                        name="si_national"
                  ...
                  <Col md={6} style={{ paddingRight: "20px" }}>
                    <Form.Group className="mb-4">
                      <Form.Label
                        ...
                        style={{ fontSize: "16px", color: "#858796" }}
                      >
                        Lien <span style={{ color: "black" }}>*</span>
                      </Form.Label>
                      <Form.Control
                        type="text"
                        name="lien"
...
const AvisInfos = ({ avisInfos = [], clientId }) => {
  const [showAddModal, setShowAddModal] = useState(false);
  ...
  const [si_fixe, setSi_fixe] = useState("1");
  ...
  const handleTypeChange = (event) => {
    ...
    setSi_fixe(newValue);
    console.log("Type d'offre changé en :", newValue === "1" ? "Non" : "Oui");
  };
  ...
  const filterAvis = () => {
    const etat = $("#etat").val();
    const table = $("#avisInfosTable").DataTable();

    table.columns().search("").draw();

    if (etat !== "all") {
      if (etat === "2") {
        table.column(4).search("false").draw(); // si_valider = false
      } else if (etat === "3") {
        table.column(4).search("true").draw(); // si_valider = true
      } else if (etat === "10") {
        table.column(5).search("true").draw(); // si_archiver = true
      }
    }
  };
  ...
  useEffect(() => {
    ...
    if (avisInfos.length > 0) {
      ...
      $("#avisInfosTable tbody").on("click", "button", function () {
        const offreId = $(this).data("id");
        const action = $(this).data("action");
        const offre = avisInfos.find((o) => o.id === offreId);
  ...
  return (
    <section className="container mt-4">
      ...
      <Modal show={showAddModal} onHide={handleCloseAddModal} size="xl">
        <Modal.Header closeButton>
          <Modal.Title style={{ color: "#858796" }}>Nouveau avis</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Form onSubmit={handleSubmit}>
            <input
              type="hidden"
              name="_token"
              value="za8y7uqCskRLxgUgjuLj1jdcdvP2JpDm2fRLqdES"
            />
            ...
            <Form.Group className="mb-3 ms-1" style={{ paddingRight: "10px" }}>
              <Form.Label
                className="ms-"
                style={{ fontSize: "16px", color: "#858796" }}
              >
                Lien <span className="">*</span>
              </Form.Label>
              <Form.Control
                type="text"
                name="lien"
