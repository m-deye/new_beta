                  ...
                  >
                    <p
                      ...
                      dangerouslySetInnerHTML={{
                        __html: groupement_spacial === "oui" ? titre_groupement_cpacial : titre,
                      }}
                    ></p>
                  </a>
                </div>
    ...
    const fetchOffres = async () => {
      try {
        ...
        const response = await axiosInstance.get(
          `/offres_emploi/liste/?lang=${i18n.language}&listes=complet`,
          { params }
        );
            ...
            <div className="row px-lg-4 px-0">
              {offres.length === 0 ? (
                ...
                offres.flatMap((offre, index) => {
                  const cards = [];
                  // Carte de l'offre principale (ou non groupée)
                  const mainDT = splitDateTime(offre.date_limite);
                  cards.push(
                    <OffreCard
                      key={`main-${offre.id}-${index}`}
                      logo={offre.client__logo}
                      entreprise={offre.groupement_spacial === "oui" ? offre.titre_entreprise : offre.client__nom}
                      titre={offre.titre}
                      date={mainDT.date}
                      time={mainDT.time}
                  ...
                  // Si offre principale: afficher aussi ses offres liées
